...
@torch.no_grad()
def log_images(generator, val_loader, epoch, cfg, num_samples=4):
    generator.eval()
...
def validate(generator, val_loader):
    generator.eval()
    total_psnr, total_ssim, total_l1, count = 0.0, 0.0, 0.0, 0
    l1 = nn.L1Loss()
    ...
    for lr, hr in val_loader:
        ...
        total_l1 += l1(sr, hr).item()
        count += 1

    generator.train()
    return total_psnr / count, total_ssim / count, total_l1 / count


def pretrain(epochs, generator, optimizer, train_loader, val_loader, csv_writer, cfg):
    ...
    for epoch in range(epochs):
        ...
        avg_psnr, avg_ssim, avg_l1 = validate(generator, val_loader)
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            log_images(generator, val_loader, epoch + 1, cfg)

    torch.save({'generator': generator.state_dict()}, f'{cfg.checkpoint_dir}/pretrain_final.pt')


def train_gan(epochs, generator, discriminator, opt_g, opt_d, train_loader, val_loader, csv_writer, cfg):
    ...
    for epoch in range(epochs):
        ...
        for lr, hr in pbar:
            ...
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                ...
                g_adv = criterion.generator_loss(real_logits, fake_logits)
...
if __name__ == "__main__":
    ...
    csv_path = f'{cfg.checkpoint_dir}/training_log.csv'
    csv_fields = ['phase', 'epoch', 'loss', 'psnr', 'ssim', 'lr', 'd_loss', 'g_loss', 'g_adv', 'g_pix', 'g_ssim']
    ...
    if os.path.exists(pretrain_path):
        ...
        pretrain(cfg.pretrain_epochs, generator, opt_g, train_loader, val_loader, csv_writer, cfg)

    if cfg.gan_epochs > 0:
        opt_g = torch.optim.AdamW(generator.parameters(), lr=cfg.lr_g)
        opt_d = torch.optim.AdamW(discriminator.parameters(), lr=cfg.lr_d)
        train_gan(cfg.gan_epochs, generator, discriminator, opt_g, opt_d, train_loader, val_loader, csv_writer, cfg)
